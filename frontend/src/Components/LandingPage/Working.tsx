...
const Working = () => {
    return <div className="mt-20 pb-5 overflow-hidden">
        <div data-aos="zoom-out" className="text-4xl md-mx:text-3xl sm-mx:text-2xl xs-mx:text-xl text-center font-semibold mb-3 text-mine-shaft-100">
            How it <span className="text-ocean-blue-400">Works</span>
        </div>
        <div className="flex px-16 bs-mx:px-10 gap-2 md-mx:flex-col md-mx:px-5 justify-between items-center">
            <div data-aos="fade-right" className="relative">
                <img className="w-[30rem]" src="/Working/Girl.png" alt="girl" loading="lazy" />
                <div className="w-36 xs-mx:w-28 flex top-[15%] right-0 absolute flex-col items-center gap-1 border border-ocean-blue-400 rounded-xl py-3 px-1 backdrop-blur-md">
                    <Avatar className="!h-16 !w-16 xs-mx:!h-12 xs-mx:!w-12" src="avatar1.png" alt="avatar" />
            ...
            <div data-aos="fade-left" className="flex flex-col gap-10">
                {work.map((item, index) => 
                    <div key={index} className="flex items-center gap-4">
                        <div className="p-2.5 bg-ocean-blue-300 rounded-full">
                            <img className="h-12 w-12 md-mx:w-9 md-mx:h-9 sm-mx:w-7 sm-mx:h-7" src={`/Working/${item.img}`} alt={item.name} loading="lazy" />
                        </div>
                        <div>
